    ...
    try:
        ...
        if 'SQL compilation error' in error_message and 'invalid identifier' in error_message:
            ...
            while retry_count < max_retries:
                ...
                if not match:
                    print("Failed to extract column names from error message.")
                ...
                for column_name in match:
                    try:
                        ...
                        add_column_query = f"ALTER TABLE IVBI.ODS_TEST.{each_table} ADD COLUMN {column_name} {snowflake_data_type}"
                ...
                try:
                    ...
                    if not O:
                        raise Exception("Data insertion failed after adding missing columns.")
                    ...
                    if 'invalid identifier' in error_message:
                        print(f"[Retry {retry_count + 1}] New invalid identifier found, will retry...")
                        retry_count += 1
                        continue
            ...
            if not success:
                raise Exception(f"Failed to insert data after {max_retries} retries due to missing columns.")
