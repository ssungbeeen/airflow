...
def execute_view_query_and_send_email(**kwargs):
    ...
    MART_query = f"""SELECT * FROM IVBI.LOG_TABLE.V_CHECK_MART_D"""
    conn = snowflake_hook.get_conn()
    cursor = conn.cursor()
    cursor.execute(MART_query)
    MART_CHECK = cursor.fetchall()
    columns = [col[0] for col in cursor.description]
    
    MART_CHECK = pd.DataFrame(MART_CHECK, columns = ['AGG_LEVEL','DIV','JOB','STATUS','ROWS','START_TIME','RUN_TIME', 'ERROR'])

    MART_CHECK['START_TIME'] = pd.to_datetime(MART_CHECK['START_TIME'], format='%H:%M:%S').dt.time
    MART_TOTAL_RUN_TIME = datetime.combine(datetime.min, max(MART_CHECK['START_TIME'])) - datetime.combine(datetime.min, min(MART_CHECK['START_TIME']))
    
    # Calculate success and total count
    success_count = len(MART_CHECK[MART_CHECK['STATUS'] == 'O'])
    total_count = len(MART_CHECK)
    
    # Create the email message
    message = f"""
    <h3>★IVBI MART 집계 모니터링★</h3>
    <p><b>성공:</b> {success_count} / <b>전체:</b> {total_count}</p>
    <p><b>소요시간:</b> {MART_TOTAL_RUN_TIME}</p>
    <h4>Details:</h4>
    {MART_CHECK.to_html(index=False)}
    """
    ...
    email_operator = EmailOperator(
        task_id='send_email',
        to='IVBI@eland.co.kr',
        subject=f"★IVBI MART 집계 모니터링★_{datetime.now().strftime('%Y%m%d')}",
