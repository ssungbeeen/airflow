...
dag = DAG(
    'BI_STOCKACCT_PROD_MMART_NEW_CONDITION',
...
def AGG_TABLE():
    test_query = """
    WITH TEST AS (
        SELECT LT.CLOSE_YYYY_MM, ISNULL(RT.BI_CLOSE_YN, 'N') AS BI_CLOSE_YN, RT.BI_CLOSE_DAY
        FROM (
        SELECT FORMAT(DATEADD(MONTH, - (n - 1), CAST(CONVERT(VARCHAR(7), GETDATE(), 120) + '-01' AS DATE)), 'yyyy-MM') AS CLOSE_YYYY_MM
        FROM (
        SELECT TOP 24 ROW_NUMBER() OVER (ORDER BY (SELECT NULL)) AS n 
        FROM master.dbo.spt_values) AS TMP ) AS LT
    LEFT JOIN ISM.dbo.TLS_TAX_CLOSE RT
        ON LT.CLOSE_YYYY_MM = RT.CLOSE_YYYY_MM
    WHERE CAST(LEFT(LT.CLOSE_YYYY_MM, 4) AS INT) >= 2024
        )
    SELECT * FROM TEST
    WHERE (BI_CLOSE_YN = 'N' OR (BI_CLOSE_YN = 'Y' AND CONVERT(VARCHAR(10), BI_CLOSE_DAY, 120) = CONVERT(VARCHAR(10), DATEADD(DAY, -1, GETDATE()), 120))) ORDER BY CLOSE_YYYY_MM"""
    
    from_cursor.execute(test_query)
    result = from_cursor.fetchall()
    CLOSE_YYYY_MM = result[0][0]
    
    START_DATE = f"{CLOSE_YYYY_MM}-01"
    ...
    try :
        success, nchunks, nrows, _ = write_pandas(conn=to_conn
                                              ,df=STOCKACCOUT
                                              ,table_name= 'BI_STOCKACCT_PROD_1D'
                                              ,database='IVBI'
                                              ,schema='MART'
        ...
        row_count_query = f"""
                        SELECT COUNT(*) 
                        FROM IVBI.MART.BI_STOCKACCT_PROD_1D WHERE TO_CHAR(MIG_DATETIME,'YYYY-MM-DD') = TO_CHAR(CURRENT_TIMESTAMP,'YYYY-MM-DD')
                    """
        ...
        STATUS = 'O'
        ERROR = 'NULL'
        to_cursor.execute(f'''INSERT INTO IVBI.LOG_TABLE.JOB_LOG_MART VALUES ('BI_STOCKACCT_PROD_1D','{start}', '{end}', '{STATUS}', '{row_count}','D','{ERROR}');''')
    except Exception as ERROR:
                    print('업로드 오류 발생 Error Message:', ERROR)
                    start = datetime.now()
                    STATUS = 'X'
                    to_cursor.execute(f'''INSERT INTO IVBI.LOG_TABLE.JOB_LOG_MART VALUES ('BI_STOCKACCT_PROD_1D', '{start}', NULL, '{STATUS}','0','D','{ERROR}');''')
...
truncate_sql_1D = """
TRUNCATE BI_STOCKACCT_PROD_1D;
"""

delete_old_records_sql_1D = """
DELETE FROM BI_STOCKACCT_PROD_1D WHERE CAST(MIG_DATETIME AS DATE) < CAST(GETDATE() AS DATE);
"""



update_mig_date_sql = """
INSERT INTO BI_STOCKACCT_PROD_M
SELECT * FROM IVBI.MART.BI_STOCKACCT_PROD_1D;
"""

delete_old_records_sql_M = """
DELETE FROM BI_STOCKACCT_PROD_M
WHERE MIG_DATETIME IN (
    SELECT MIG_DATETIME
    FROM (
        SELECT
            MIG_DATETIME,
            ROW_NUMBER() OVER (PARTITION BY "상품코드", "품명 규격", START_DATE, END_DATE
                               ORDER BY MIG_DATETIME DESC) AS row_num
        FROM BI_STOCKACCT_PROD_M
    ) AS SubQuery
    WHERE row_num > 1
);"""
...
delete_old_records_task_1D = SnowflakeOperator(
    task_id='DELETE_OLD_RECORDS_1D',
    snowflake_conn_id='snowflake_default_5',
...
truncate_task_1D >> AGG_TABLE >> delete_old_records_task_1D  >> load_data_task >> delete_old_records_task_M
