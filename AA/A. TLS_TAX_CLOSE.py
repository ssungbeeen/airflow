...
dag = DAG(
    ...
    description='미수금 프로시저 값 정보 비교를 위한 ODS TLS_TAX_CLOSE',
    start_date=pendulum.datetime(2025, 8, 7, tz="Asia/Seoul"),
    schedule_interval='45 0 * * 1-7', 
    catchup=False,
)
...
job_info_query = "SELECT * FROM IVBI.LOG_TABLE.JOB_INFO WHERE AGG_LEVEL = '0' AND DEL_YN != 'Y' AND SCHEDULE = 'D' AND (JOB_CODE = 'TLS_TAX_CLOSE')"
